...
(function($) {
	...
	$.fn.fnReplaceInputBoxes = function() {
        ...
        if(objForm.find("#Password").length) {
            objForm.find("#Password").addClass('hiddenPwd');
            var toggle = _a("id:PasswordVisibility", "onclick:togglePassword()", "text:{{Show}}");
            objForm.find("#Password").after(toggle.toHTML());
            $($("#Password")[0].parentNode).css("display", "flex");
            $($("#Password")[0]).css({"flex-grow" : "1", "width" : ""});
        }
	}
	    ...
	    $(objForm).find('select').each(function (i, ctrl) {
	    	...
	    	if($(ctrl).css('width') != 'auto' && $(ctrl).parent().css('width') != 'auto') {
	    		var per = Math.ceil( 100 * parseFloat($(ctrl).css('width')) / parseFloat($(ctrl).parent().css('width')) +3 );
                        ...
                        width = ""+ ((_.isNaN(per)|| !isFinite(per))?"25":per) + '%';
	    	}
	...
	$.fn.fnReplaceFiles = function() {
		...
		$(objForm).find('input[type="file"]').each(function (i, ctrl) {
	    	...
	    	/* Define Events  */
                $(ctrl).bind('change', function () { $(nCtrl).find('cite').text($(this).val().replace(/^.*\\/, "")); });
